...
fn test_save_invalid_key_to_safetensors_file() {
    ...
    map.insert(
        ...
        vec![Tensor::randn(&[10], (Kind::Float, tch::Device::Cpu))],
    );
...
fn test_load_safetensors_file_and_verify_dtypes() {
    ...
    let (ds_i64, original_map_i64) = setup_typed_dataset(
        4,
        &[2],
        "i64_key",
        &(0..8).collect::<Vec<i64>>(),
    ...
    ds_i64
        .save_to_file(&file_path_i64)
        .expect("Save i64 failed");
    ...
    // Spot check
    assert_eq!(
        loaded_tensors[0].kind(),
        Kind::Int64,
        "Should be int64 kind"
    );
    ...
    ds_bool
        .save_to_file(&file_path_bool)
        .expect("Save bool failed");
    let loaded_bool =
        SafetensorsDataset::load_from_file(&file_path_bool).expect("Load bool failed");
